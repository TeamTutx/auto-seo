"""Scheduled automated re-audits + alerts (REQUIREMENTS.md §2.7, §3.1) -
a Pro/Agency perk; free plan only gets manual, throttled rescans. Email
delivery needs a Resend/SendGrid key we don't have, so alerts land in-app
...
from app.models import Alert, AlertType, Audit, Check, CheckStatus, Page, PlanTier, Site, User
...
def run_scheduled_audits(session: Session) -> int:
    """Re-audit every page belonging to a Pro/Agency user; raise an Alert on
    a meaningful regression (score drop or a newly-failing check). Returns
    the number of alerts created. A single page's fetch failure doesn't
    abort the rest of the run."""
    paid_page_ids = session.exec(
        select(Page.id)
